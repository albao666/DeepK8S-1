...
class Brain(tf.keras.Model):
    ...
    def call(self, state):
        ...
        state = self.dense_4(state)
        state = self.dense_5(state)
        # state = tf.layers.dense(state, 3, activation=tf.tanh)
        # state = tf.layers.dense(state, 9, activation=tf.tanh)
        # state = tf.layers.dense(state, 18, activation=tf.tanh)
        # state = tf.layers.dense(state, 9, activation=tf.tanh)
        # state = tf.layers.dense(state, 1)
        return tf.expand_dims(tf.squeeze(state, axis=-1), axis=0)
